...
int main()
{
    ...
    coordinates[0] = getRandomPosition();
    coordinates[1] = getRandomPosition();
    ...
    while (1) // looping infinte times
    {
        ...
        roomnumber = coordinates[3]; // setting user room number
        ...
        {
          if(room==adjacent[roomnumber-1].x ||
            room==adjacent[roomnumber-1].y ||
            room==adjacent[roomnumber-1].z)
            {
              coordinates[3]=room;
              if (coordinates[2] == room)
              {
                if (coordinates[2] % 2 != 0)
                {
                  printf("You hear a slithering sound, as the Wumpus slips away. \n");
                  printf("Whew, that was close! \n");
                  coordinates[2] = adjacent[room - 1].x;
                }
                else
                {
              ...
              if (adjacent[room - 1].x == coordinates[0] ||
                 adjacent[room - 1].y == coordinates[0] ||
                 adjacent[room - 1].z == coordinates[0] ||
                 adjacent[room - 1].x == coordinates[1] ||
                 adjacent[room - 1].y == coordinates[1] ||
                 adjacent[room - 1].z == coordinates[1])
                {
                  strcpy(message, "You feel a draft. ");
                }

                if (adjacent[room - 1].x == coordinates[2] ||
                ...
                if ((adjacent[room - 1].x == coordinates[2] ||
                    adjacent[room - 1].y == coordinates[2] ||
                    adjacent[room - 1].z == coordinates[2]) &&
                    (adjacent[room - 1].x == coordinates[0] ||
                    adjacent[room - 1].y == coordinates[0] ||
                    adjacent[room - 1].z == coordinates[0] ||
                    adjacent[room - 1].x == coordinates[1] ||
                    adjacent[room - 1].y == coordinates[1] ||
                    adjacent[room - 1].z == coordinates[1]))
                  {
                   ...
                   // CHECKING FOR PITS
                  if (coordinates[0] == room || coordinates[1] == room)
                  {
        ...
        else if (command[0] == 'R' || command[0] == 'r') // if reset command
        {
          ...
          scanf("%d  %d  %d  %d", &person,&wumpus,&pit1,&pit2); // taking input from user

          coordinates[0]=pit1;
          coordinates[1]=pit2;
          coordinates[2]=wumpus;
          coordinates[3]=person;
...
void resetGame(int *data)
{
    int coordinates[4] = {0}; // declaring an array to store the coordinates
    printf("Enter the room locations (1..20) for player, wumpus, pit1, and pit2: "
           "\n"); // printing the message
    scanf("%d %d %d %d", &coordinates[3], &coordinates[2], &coordinates[0],
          &coordinates[1]); // taking input from user

    for (int i = 0; i < 4; i++) // copying data
    {
...
int getRandomPosition()
{
    int number =(rand() % 20) + 1; //(1 - 20 + 1)) + 1; // generates random number between 1 and 20
